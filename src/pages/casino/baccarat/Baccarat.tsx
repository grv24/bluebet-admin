...
const BaccaratComponent: React.FC<BaccaratProps> = ({
  casinoData,
  remainingTime,
  onBetClick,
  results = [],
            ...
            {[
              ...
              return (
                <div key={index} className="flex flex-col items-center">
                  <div
                    className={`flex flex-col w-26  items-center justify-center text-white px-2 bg-[var(--bg-secondary)] relative cursor-pointer `}
                    onClick={() => !locked && handleBetClick(data)}
                  >
                    <h2 className="md:text-sm text-[10px] font-semibold text-nowrap">
                      {item}
                    </h2>
                    <h2 className="text-sm font-semibold">{data?.b ?? 0}:1</h2>
                    {locked && (
...
const Baccarat = memoizeCasinoComponent(BaccaratComponent);
