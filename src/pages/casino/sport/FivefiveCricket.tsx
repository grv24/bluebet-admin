...
const FivefiveCricketComponent: React.FC<FivefiveCricketProps> = ({
  ...
  // win "1" = AUS (Australia), "2" = IND (India)
  const getResultDisplay = (win: string) => {
    const winMap: {
      [key: string]: { label: string; color: string; title: string };
    } = {
      "1": { label: "A", color: "text-red-500", title: "Australia" },
      "2": { label: "I", color: "text-yellow-500", title: "India" },
      "0": { label: "T", color: "text-white", title: "Tie" },
    };
        ...
        {bookmakerMarket && (
          <div className="bg-[var(--border)]/20 w-full">
            ...
            {/* <div className="text-xs font-bold text-[var(--bg-primary90)] pl-2 py-1">
              Min: {bookmakerMarket.min || 100} Max: {formatMax(bookmakerMarket.max || 300000)}
            </div> */}
            <table className="w-full">
              <thead>
                <tr>
                  ...
                  <td>
                    <div className="text-center text-sm md:text-base py-1 bg-[var(--bg-lay)] font-semibold">
                      Lay
                    </div>
                  </td>
                </tr>
            ...
            return (
              <div key={`fancy-market-${market.mid || marketIdx}`}>
                ...
                {/* {(market.min || market.max) && (
                  <div className="text-xs font-bold text-[var(--bg-primary90)] pl-2 py-1">
                    Min: {market.min || 100} Max: {formatMax(market.max || 100000)}
                  </div>
                )} */}
      ...
      {results && results.length > 0 && (
        <div className="mt-1 flex flex-col gap-1">
          <div className="bg-[var(--bg-secondary85)] border border-gray-300 flex justify-between px-2 py-0.5 items-center">
            ...
            <button
              onClick={() => navigate(`/casino-result?game=CRICKET_V3`)}
...
const FivefiveCricket = memoizeCasinoComponent(FivefiveCricketComponent);
FivefiveCricket.displayName = "FivefiveCricket";

export default FivefiveCricket;
